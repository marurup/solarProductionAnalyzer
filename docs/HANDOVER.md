...
- **Importér energidata fra solcelle-inverteren** (Growatt, Huawei/SolarEdge m.fl., og aggregatorer som [solar-assistant.io](https://solar-assistant.io)). Inverteren har finere/mere komplet data end eloverblik (fx produktion FØR nettomåling, selvforbrug per interval, batteri-flow). Formål: **beregn den reelle besparelse ved at have solceller** — sammenlign inverterens produktions-/selvforbrugstal mod de aktuelle spot+tarif-priser time for time (selvforbrugt kWh × undgået købspris + eksporteret kWh × nettopris). Afklaring: hvilke eksportformater/API'er (Growatt ShinePhone/OSS API, solar-assistant CSV/MQTT-eksport), tidsopløsning, og hvordan de mappes til appens time-model. Kan udvide den eksisterende parser eller blive en ny import-kilde ved siden af eloverblik-CSV.
...
- ✅ **Produktionsaftager-dropdown (v1)** — i Salg-panelet en `<select id="aftagerSelect">` over kendte elhandlere der køber overskudsstrøm; valg udfylder `t4` (elhandlerens spread/gebyr) og viser note med kilde + dato. "— Indtast selv —" lader brugeren skrive selv, og manuel redigering af t4 nulstiller valget. Listen er en **manuelt vedligeholdt** inline-const `PRODUKTIONSAFTAGERE` (mellem `==PRODUKTIONSAFTAGERE_START==`/`_END==`-markørerne) — ingen fetch, ingen fejl-modes. Fast disclaimer i UI'et om at listen kan være forældet. **Kun kilde-bakkede tal medtaget** (ingen opdigtede gebyrer): Vindstød 1 øre/kWh (egen afregning, verificeret 20 mdr), Norlys 0 øre (som Norlys-elkunde) / 0,80 øre (kunde andetsteds) fra norlys.dk. **Udvidelse:** tilføj poster i const'en efterhånden som vilkår verificeres (Modstrøm, Andel, NRGi, EWII m.fl. — deres spread/balancegebyr var ikke pålideligt tilgængeligt ved research 2026-08; de tilbyder spot + 0 kr abonnement, men selve gebyret skal bekræftes). Model dækker p.t. fast øre/kWh-spread; % af spot og produktions-abonnement (revenueAbon) kunne tilføjes.

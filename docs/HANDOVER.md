...
- **Omstrukturér trin-inddelingen (1, 1½, 2 → 1, 2, 3).** Et "trin 1½" (adresse-quick-konfig, `index.html:347`) er ulogisk. Ny inddeling: **Trin 1: Importér data** (eloverblik-CSV upload). **Trin 2: Sæt tariffer** — både import/køb (forbrugstariffer + markup) OG eksport/salg (produktionsfradrag t1–t4). Adresse-quick-konfig hører hjemme under trin 2 som en genvej til at udfylde tarifferne, ikke som et separat halvtrin. **Trin 3: Analysér.** Opdatér `#stepIndicator` (allerede 1/2/3), fjern "1½"-overskriften, og flyt adresse-boksen ind under tarif-trinnet.
